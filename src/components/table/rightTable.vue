<template>
	<div class="table-container">
		<div class="header-menu mb10" v-if="setHeaderTitle">
			<el-text class="mx-1" size="large">{{ headerMenu.name }}</el-text>
			<div class="buttonList">
				<div v-for="(val, key) in button" :key="key" class="ml5">
					<el-button :type="val.type" :icon="val.icon === 'Plus' ? Plus : ''" @click="submitEvent(val)">{{ val.text }}</el-button>
				</div>
			</div>
		</div>
		<el-table
		...
		>
			<el-table-column type="selection" align="center" :reserve-selection="true" width="50" v-if="config.isSelection" />
			...
			<el-table-column label="操作" align="center" width="200" v-if="config.isSwitch">
				<template v-slot="scope">
					<el-text class="mx-1" type="success" v-if="scope.row.switch === 'true'">成功</el-text>
					<el-text class="mx-1" type="danger" v-else>失败</el-text>
				</template>
			</el-table-column>
			<el-table-column label="操作" align="center" width="200" v-if="config.isOperate">
				<template v-slot="scope">
		...
		<div class="table-footer mt15">
			...
			<div class="table-footer-tool" v-if="false">
...
// 定义子组件向父组件传值/事件
const emit = defineEmits(['delRow', 'pageChange', 'sortHeader', 'addSource']);
...
// 设置边框显示/隐藏
const setHeaderTitle = computed(() => {
	return props.config.isShowTitle ? true : false;
});
// 设置边框显示/隐藏
